...
def single_scale_retinex(image, scale=15, alpha=-1, gamma=1, epsilon=1e-3):
    # Convert the image to float32 and add a small constant to avoid log(0)
    img_float = np.float32(image) + epsilon

    # Apply Gaussian blur
    blurred = cv2.GaussianBlur(img_float, (scale, scale), 0) + epsilon

    # Apply the Retinex formula
    ssr_img = np.log(img_float) - np.log(blurred)

    # Scale the image to control brightness
    ssr_img = ssr_img * alpha

    # Normalize to the range [0, 255]
    ssr_img = cv2.normalize(ssr_img, None, 0, 255, cv2.NORM_MINMAX)
    
    # Convert to uint8
    ssr_img = np.uint8(ssr_img)

    # Apply gamma correction
    ssr_img = np.power(ssr_img / 255.0, gamma) * 255
    ssr_img = np.uint8(ssr_img)
...
# Deblurring with Wiener Filter for color images
def apply_wiener_filter(image):
    # Split the image into its color channels
    b, g, r = cv2.split(image)

    # Apply Wiener filter to each channel
    deblurred_b = wiener(b, (5, 5))
    deblurred_g = wiener(g, (5, 5))
    deblurred_r = wiener(r, (5, 5))

    # Merge the channels back together
    deblurred_image = cv2.merge((deblurred_b.astype(np.uint8), deblurred_g.astype(np.uint8), deblurred_r.astype(np.uint8)))
...
@app.route('/advance-edit')
def advance_edit():
    return render_template('advance-edit.html')

@app.route('/deblur-image')
def deblur_image():
    return render_template('deblur-image.html')

@app.route('/low-Light')
def low_Light():
    return render_template('low-Light.html')
...
def deblur_image_post():
    image_file = request.files['image']
    npimg = np.frombuffer(image_file.read(), np.uint8)
    image = cv2.imdecode(npimg, cv2.IMREAD_COLOR)

    # Apply Wiener filter to deblur the image
    deblurred_image = apply_wiener_filter(image)

    # Encode the processed image
    _, buffer = cv2.imencode('.jpg', deblurred_image)
    encoded_image = base64.b64encode(buffer).decode('utf-8')

    return 'data:image/jpeg;base64,' + encoded_image


@app.route('/low-Light', methods=['POST'])
def single_scale_retinex_image():
    image_file = request.files['image']
    npimg = np.frombuffer(image_file.read(), np.uint8)
    image = cv2.imdecode(npimg, cv2.IMREAD_COLOR)

    # Apply Single Scale Retinex
    enhanced_image = single_scale_retinex(image, scale=15, alpha=5.5)  # Adjust alpha as needed
